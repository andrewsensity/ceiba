...
class GetUsersRepositoryImplTest {
    private lateinit var getUsersRepositoryImpl: GetUsersRepositoryImpl
    ...
    fun setUp() {
        ...
        getUsersRepositoryImpl = GetUsersRepositoryImpl(ceibaApi)
    }
    ...
    fun `Get posts by user id, valid response, returns success`() = runBlocking {
        mockWebServer.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody(Utils.readJsonFile("200_get_users.json"))
        )
        val result = getUsersRepositoryImpl.getUsers()
